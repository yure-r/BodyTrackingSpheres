...
function updateSpheres(data){
  ...
  CLAVICLE_LEFT.position.y = ((shoulderneckavg + data[11].y)/2)*multiplier
  CLAVICLE_RIGHT.position.y = ((shoulderneckavg + data[12].y)/2)*multiplier  
  ...
  SPINE_CHEST.position.y = ((shoulderneckavg + neckpelvisavg)/2)*multiplier //maybe that's supposed to be neck????
	...
	SHOULDER_LEFT.position.y = ((data[11].y))*multiplier
    
  ELBOW_LEFT.position.y = ((data[13].y))*multiplier
	WRIST_LEFT.position.y = ((data[15].y))*multiplier
    
	HAND_LEFT.position.y = ((data[19].y + data[15].y + data[17].y)/3)*multiplier
    
	HANDTIP_LEFT.position.y = (data[19].y)*multiplier
	THUMB_LEFT.position.y = (data[21].y)*multiplier
	
	SHOULDER_RIGHT.position.y = (data[12].y)*multiplier
	ELBOW_RIGHT.position.y = (data[14].y)*multiplier
    
	WRIST_RIGHT.position.y = (data[16].y)*multiplier
    
	HAND_RIGHT.position.y = ((data[16].y + data[18].y + data[20].y)/3)*multiplier
    
	HANDTIP_RIGHT.position.y = (data[20].y)*multiplier
	THUMB_RIGHT.position.y = (data[22].y)*multiplier
 
	KNEE_LEFT.position.y = (data[25].y)*multiplier
	ANKLE_LEFT.position.y = (data[27].y)*multiplier
	FOOT_LEFT.position.y = (data[31].y)*multiplier
    
  HIP_LEFT.position.y = (data[23].y)*multiplier
	HIP_RIGHT.position.y = (data[24].y)*multiplier
    
	KNEE_RIGHT.position.y = (data[26].y)*multiplier
	ANKLE_RIGHT.position.y = (data[28].y)*multiplier
	FOOT_RIGHT.position.y = (data[32].y)*multiplier
	
	NOSE.position.y = (data[0].y)*multiplier    //[0]
	EYE_LEFT.position.y = ((data[1].y + data[2].y + data[3].y)/3)*multiplier 
	EAR_LEFT.position.y = (data[7].y)*multiplier
	EYE_RIGHT.position.y = ((data[6].y + data[5].y + data[4].y)/3)*multiplier
	EAR_RIGHT.position.y = (data[8].y)*multiplier
